...
module.exports = async ctx => {
 ...
 // let level = query.level
  ...
  try {
    var result = []
    //行政简讯
    var administrative_sql = "select id,title,summary,content,publisher,level,DATE_FORMAT(publish_time, '%Y-%m-%d %H:%i:%S') as publish_date,pictures from client_notice where level > 0 and notice_type = 1 order by publish_date desc LIMIT 1"
